...
int main(int argc, char *argv[])
{
    ...
    /* harder to exploit buffer overlow vulnerability on heap */
    ...
    while(1) {
        ...
        while(1) {
            ...
            /* I have shown below different ways to handle the Buffer Overflow Vulnerability*/
            
            // Will Exit if buffer is greater than max data size
            if(strlen(recvBuff) > MAX_DATA_SIZE){
                printf("Error. Buffer Overflow Warning. Exiting.");
                exit(1);
            }
            
            /* Another way is to use strncpy instead of strcpy. strncpy will only copy the first
               n letters, where n is the MAX_DATA_SIZE, and will ignore everything else*/
