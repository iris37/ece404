...
def substitute( expanded_half_block ):
    '''
    Recall that in round processing the left-half of the 64-bit input block passes
    through unchanged to the output where it becomes the right-half for the next
    round.  On the other hand, the right-half is first expanded from 32-bits to 48
    bits.  The job of this method is break the 48 bits into eight segments of 6 bits
    each and to then apply the substitution rule separately to each of the 8 segments
    using the s-box meant specifically for that segment.
    '''
    output = BitVector (size = 32)
    segments = [expanded_half_block[x*6:x*6+6] for x in range(8)]
    for sindex in range(len(segments)):
        row = int(segments[sindex][0] + segments[sindex][-1])
...
right_half_32bits = BitVector( intVal = 17, size = 32 )
